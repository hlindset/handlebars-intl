(function() {
    ...
    var $$utils2$$hop = Object.prototype.hasOwnProperty;

    function $$utils2$$extend(obj) {
        var sources = Array.prototype.slice.call(arguments, 1),
            i, len, source, key;
    ...
    var $$es53$$realDefineProp = (function () {
        try { return !!Object.defineProperty({}, 'a', {}); }
        catch (e) { return false; }
    })();

    var $$es53$$es3 = !$$es53$$realDefineProp && !Object.prototype.__defineGetter__;

    var $$es53$$defineProperty = $$es53$$realDefineProp ? Object.defineProperty :
            function (obj, name, desc) {

        if ('get' in desc && obj.__defineGetter__) {
            obj.__defineGetter__(name, desc.get);
        } else if (!$$utils2$$hop.call(obj, name) || 'value' in desc) {
            obj[name] = desc.value;
        }
    };

    var $$es53$$objCreate = Object.create || function (proto, props) {
        var obj, k;
        ...
        for (k in props) {
            if ($$utils2$$hop.call(props, k)) {
                $$es53$$defineProperty(obj, k, props[k]);
            }
        }

        return obj;
    };
    var $$compiler1$$default = $$compiler1$$Compiler;

    function $$compiler1$$Compiler(locales, formats, pluralFn) {
        this.locales  = locales;
        this.formats  = formats;
        this.pluralFn = pluralFn;
    }

    $$compiler1$$Compiler.prototype.compile = function (ast) {
        this.pluralStack        = [];
        this.currentPlural      = null;
        this.pluralNumberFormat = null;
    ...
    $$compiler1$$Compiler.prototype.compileMessage = function (ast) {
        if (!(ast && ast.type === 'messageFormatPattern')) {
            throw new Error('Message AST is not of type: "messageFormatPattern"');
        }
    ...
    $$compiler1$$Compiler.prototype.compileMessageText = function (element) {
        // When this `element` is part of plural sub-pattern and its value contains
        // an unescaped '#', use a `PluralOffsetString` helper to properly output
        // the number with the correct offset in the string.
        if (this.currentPlural && /(^|[^\\])#/g.test(element.value)) {
            ...
            return new $$compiler1$$PluralOffsetString(
                    this.currentPlural.id,
                    this.currentPlural.format.offset,
                    this.pluralNumberFormat,
                    element.value);
    ...
    $$compiler1$$Compiler.prototype.compileArgument = function (element) {
        var format = element.format;

        if (!format) {
            return new $$compiler1$$StringFormat(element.id);
        }
        ...
        switch (format.type) {
            ...
            case 'pluralFormat':
                options = this.compileOptions(element);
                return new $$compiler1$$PluralFormat(element.id, format.offset, options, pluralFn);

            case 'selectFormat':
                options = this.compileOptions(element);
                return new $$compiler1$$SelectFormat(element.id, options);
    ...
    $$compiler1$$Compiler.prototype.compileOptions = function (element) {
        var format      = element.format,
            options     = format.options,
            optionsHash = {};
    ...
    function $$compiler1$$StringFormat(id) {
        this.id = id;
    }

    $$compiler1$$StringFormat.prototype.format = function (value) {
        if (!value) {
            return '';
        }
    ...
    function $$compiler1$$PluralFormat(id, offset, options, pluralFn) {
        this.id       = id;
        this.offset   = offset;
        this.options  = options;
        this.pluralFn = pluralFn;
    ...
    $$compiler1$$PluralFormat.prototype.getOption = function (value) {
        var options = this.options;
    ...
    function $$compiler1$$PluralOffsetString(id, offset, numberFormat, string) {
        this.id           = id;
        this.offset       = offset;
        this.numberFormat = numberFormat;
        this.string       = string;
    ...
    $$compiler1$$PluralOffsetString.prototype.format = function (value) {
        var number = this.numberFormat.format(value - this.offset);
    ...
    function $$compiler1$$SelectFormat(id, options) {
        this.id      = id;
        this.options = options;
    }

    $$compiler1$$SelectFormat.prototype.getOption = function (value) {
        var options = this.options;
        return options[value] || options.other;
    };

    var intl$messageformat$parser1$$default = (function() {
      /*
       * Generated by PEG.js 0.8.0.
       *
       * http://pegjs.majda.cz/
    ...
    var $$core2$$default = $$core2$$MessageFormat;

    // -- MessageFormat --------------------------------------------------------

    function $$core2$$MessageFormat(message, locales, formats) {
        // Parse string messages into an AST.
        var ast = typeof message === 'string' ?
                $$core2$$MessageFormat.__parse(message) : message;
        ...
        // Creates a new object with the specified `formats` merged with the default
        // formats.
        formats = this._mergeFormats($$core2$$MessageFormat.formats, formats);

        // Defined first because it's used to build the format pattern.
        $$es53$$defineProperty(this, '_locale',  {value: this._resolveLocale(locales)});

        var pluralFn = $$core2$$MessageFormat.__localeData__[this._locale].pluralRuleFunction;
    ...
    // constructor. These are used when constructing the internal Intl.NumberFormat
    // and Intl.DateTimeFormat instances.
    $$es53$$defineProperty($$core2$$MessageFormat, 'formats', {
        enumerable: true,
    ...
    // Define internal private properties for dealing with locale data.
    $$es53$$defineProperty($$core2$$MessageFormat, '__localeData__', {value: $$es53$$objCreate(null)});
    $$es53$$defineProperty($$core2$$MessageFormat, '__addLocaleData', {value: function (data) {
        if (!(data && data.locale)) {
            throw new Error(
                'Locale data provided to IntlMessageFormat is missing a ' +
                '`locale` property'
        ...
        $$core2$$MessageFormat.__localeData__[locale] = data;
    }});

    // Defines `__parse()` static method as an exposed private.
    $$es53$$defineProperty($$core2$$MessageFormat, '__parse', {value: intl$messageformat$parser1$$default.parse});

    // Define public `defaultLocale` property which defaults to English, but can be
    // set by the developer.
    $$es53$$defineProperty($$core2$$MessageFormat, 'defaultLocale', {
        enumerable: true,
        writable  : true,
        value     : undefined
    });

    $$core2$$MessageFormat.prototype.resolvedOptions = function () {
        // TODO: Provide anything else?
        return {
            locale: this._locale
        };
    ...
    $$core2$$MessageFormat.prototype._compilePattern = function (ast, locales, formats, pluralFn) {
        var compiler = new $$compiler1$$default(locales, formats, pluralFn);
        return compiler.compile(ast);
    };

    $$core2$$MessageFormat.prototype._format = function (pattern, values) {
        var result = '',
            i, len, part, id, value;
        ...
        for (i = 0, len = pattern.length; i < len; i += 1) {
            ...
            // Enforce that all required values are provided by the caller.
            if (!(values && $$utils2$$hop.call(values, id))) {
                throw new Error('A value must be provided for: ' + id);
            }
    ...
    $$core2$$MessageFormat.prototype._mergeFormats = function (defaults, formats) {
        var mergedFormats = {},
            type, mergedType;

        for (type in defaults) {
            if (!$$utils2$$hop.call(defaults, type)) { continue; }

            mergedFormats[type] = mergedType = $$es53$$objCreate(defaults[type]);

            if (formats && $$utils2$$hop.call(formats, type)) {
                $$utils2$$extend(mergedType, formats[type]);
            }
        }
    ...
    $$core2$$MessageFormat.prototype._resolveLocale = function (locales) {
        if (!locales) {
            locales = $$core2$$MessageFormat.defaultLocale;
        }
        ...
        var localeData = $$core2$$MessageFormat.__localeData__;
        var i, len, locale;
        ...
        for (i = 0, len = locales.length; i < len; i += 1) {
            ...
            if ($$utils2$$hop.call(localeData, locale)) {
                return locale;
            }
        }
    ...
    var $$en3$$default = {"locale":"en","pluralRuleFunction":function (n) {var i=Math.floor(Math.abs(n)),v=n.toString().replace(/^[^.]*\.?/,"").length;n=Math.floor(n);if(i===1&&v===0)return"one";return"other";}};

    $$core2$$default.__addLocaleData($$en3$$default);
    $$core2$$default.defaultLocale = 'en';

    var intl$messageformat1$$default = $$core2$$default;
    ...
    $$es51$$defineProperty($$core1$$RelativeFormat, '__addLocaleData', {value: function (data) {
        ...
        intl$messageformat1$$default.__addLocaleData(data);
    ...
    $$core1$$RelativeFormat.prototype._compileMessage = function (units) {
        ...
        return new intl$messageformat1$$default(message, locales);
    };
